                    ...
                    <div>
                        ...
                        <select name="payment_method" id="payment_method"
                            ...
                            @foreach(config('global.payment_method') as $key => $value)
                                <option value="{{ $key }}" @if($key === $order->payment_method) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="shipping_method" id="shipping_method"
                            ...
                            @foreach(config('global.shipping_method') as $key => $value)
                                <option value="{{ $key }}" @if($key === $order->shipping_method) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="shipping_status" id="shipping_status"
                            ...
                            @foreach(config('global.shipping_status') as $key => $value)
                                <option value="{{ $key }}" @if($key === $order->shipping_status) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-1">
                    <div>
                        <label for="order_note"
                               class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Order Note</label>
                ...
                <div class="relative overflow-x-auto my-5">
                    ...
                    <table id="ProductTbl" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach($order->order_items as $key => $order_item)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" id="orderItem{{$order_item->id}}">
                                <input
                                    type="hidden"
                                ...
                                <td class="px-6 py-4">
                                    <input
                                        ...
                                        type="number" name="products[{{ $key }}][qty]" value="{{ old('products.['.$key.'][qty]') ?? $order_item->quantity }}"  required>
                                </td>
                                <td class="px-6 py-4">
                                    <button type="button" class="remove-btn-item" onClick="remove(this, 'orderItem{{ $order_item->id }}')">
                                        <svg class="w-5 h-5 text-red-500 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20">
